psoptim <- function (par, fn, gr = NULL, ..., lower=-1, upper=1, it=it,
                     control = list()) {
  ...
  con <- list(trace = 0, fnscale = 1, maxit = it, maxf = Inf,
  ...
  #p.s <- ifelse(is.na(con[["s"]]),ifelse(p.type==0,5*floor(10+2*sqrt(npar)),40),
   #             con[["s"]]) # swarm size
  p.s <- 1000
